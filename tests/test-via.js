...
test('Full Process', async t => {
    await t
        ...
        //choose Make
        .click(makeSelect)
        .click(makeOption.withText('Ford'))

        // Engine performance
        .typeText(engineperformance, '500')


        // date of manufactor
        .typeText(dateofmanufacture, formattedDatedM3y, { replace: true })

        // number of seats
        .click(numberofseats)
        .click(numberofseatsOptions.withText('5'))

        //fuel type
        .click(fuel)
        .click(fuelOptions.withText('Petrol'))


        //List price
        .typeText(listprice, '20000', { replace: true })

        // annual mileage
        .typeText(annualmileage, '50000', { replace: true })
        ...
        .typeText(firstname, 'Max', { replace: true })
        .typeText(lastname, 'Resch', { replace: true })

        .typeText(birthdate, '06/24/1996', { replace: true })
        ...
        .typeText(zipcode, '1234', { replace: true })

        .click(occupationSelect)
        .click(occupationOption.withText('Employee'))
        ...
        .typeText(startdate, formattedDatedDP1M1D, { replace: true })

        .click(insuranceSumSelect)
        .click(insuranceSumOption.withText('7.000.000,00'))

        .click(meritratingSelect)
        .click(meritratingOption.withText('Bonus 5'))

        .click(damageinsuranceSelect)
        .click(damageinsuranceOption.withText('Partial Coverage'))
        ...
        .click(courtesycarOption.withText('No'))
        ...
        .expect(selectsilver_price.innerText).eql("331.00", "Preis passt nicht")

        .expect(selectgold_price.innerText).eql("977.00", "Preis passt nicht")

        .expect(selectplatinum_price.innerText).eql("1,917.00", "Preis passt nicht")

        .expect(selectultimate_price.innerText).eql("3,652.00", "Preis passt nicht")


        .click(selectsilverLabel)
        .expect(selectsilverRadio.checked).ok('Checkbox sollte ausgewählt sein')
        ...
        .typeText(email, "maxfh@fhe.com", { replace: true })
        .typeText(username, "mresch", { replace: true })
        .typeText(password, "12345678aB", { replace: true })
        .typeText(confirmpassword, "12345678aB", { replace: true })
